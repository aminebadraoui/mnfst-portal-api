...
from ..parsers import AvatarParser

logger = logging.getLogger(__name__)

class AvatarPerplexityClient(BasePerplexityClient):
    """Client for avatar analysis using Perplexity API."""
    
    def __init__(self):
        super().__init__()
        self.parser = AvatarParser()
    ...
    async def generate_insights(
        ...
        """Generate avatar analysis insights using Perplexity."""
        try:
            # Get prompt
            prompt = get_prompt(
                analysis_type=AnalysisType.AVATAR,
            ...
            logger.error(f"Error generating avatar analysis insights: {str(e)}", exc_info=True)
            return {
                "raw_perplexity_response": "",
                "structured_data": None
            } 
...
class ProductPerplexityClient(BasePerplexityClient):
    """Client for product analysis using Perplexity API."""
    ...
    async def generate_insights(
        ...
        """Generate product analysis insights using Perplexity."""
        try:
            # Get prompt
            prompt = get_prompt(
            ...
            logger.error(f"Error generating product analysis insights: {str(e)}", exc_info=True)
            return {
                "raw_perplexity_response": "",
                "structured_data": None
            } 
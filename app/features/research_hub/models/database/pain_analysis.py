...
class PainAnalysis(Base):
    ...
    # Relationships
    user = relationship("User", back_populates="pain_analysis")
    project = relationship("Project", back_populates="pain_analysis")

    # Indexes
    __table_args__ = (
        Index('ix_pain_analysis_user_project', 'user_id', 'project_id'),
        Index('ix_pain_analysis_project_query', 'project_id', 'query'),
        Index('ix_pain_analysis_task_id', 'task_id', unique=True),
    ) 